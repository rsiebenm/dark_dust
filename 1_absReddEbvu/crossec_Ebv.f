...
c
c      Fortran Program: crossec_Ebv: For stars assuming that about half
c      (fdark = 48.5%) of the mass in 1gramm of dust is in submicrometer
c     sized grains, consider the reddening E(B-V) and use these to compute  Av.
      
c     Compilation:
c  gfortran -ffixed-line-length-132     crossec_Ebv.f sigtDark_Ebv.f -o a.crossec_Ebv
c  gfortran -ffixed-line-length-132 -O5 crossec_Ebv.f sigtDark_Ebv.f -o a.crossec_Ebv
c
c     Description + References : Computes optical depth = dust cross
c     sections * column density of the Dark Dust model used for
c     computing the reddening/extinction curves.  More info on dust
c     model Siebenmorgen R., 2023A&A, 670A, 115

c     Description:
c     Dust along sightlines have columns Nl of nano+large (MRN) grains
c     and column Nd of the dark dust.
c     1) Nano-particles (VSG) include:
c        a) Graphite + PAH to fit 2175AA bump + far UV in the reddening
c        b) Nano-silicates for contribution to far UV reddening
c     2) Sub-micron particles (large grains) are prolates with a/b=2, IDG
c        alignment and optical constants by Demyk23 for amorphous Si (aSi)
c        and Zubko96 for aC
c     3) Micron-sized grains (Dark Dust) are fluffy spheres with 20% aC,
c        30% Si, and 50% vacuum; min Dark radius: alad
c        =adark(lad):=max(amrn(lesi+1)+amrn(lesi+1)).  
c
c     INPUT:

c     fdark : reladtive mass of dark dust, hardcoded (you need to know
c             what you do).
c      
c      in directory ./Input/:     
c      jsm12fit.inp:12 dust parameters(for fitting reddening curve 10
c                   and 2 for dust alignmenr radii). There are 5 dust
c                   abundances: abuc, abusi (fixed), abuvsi, abucvgr,
c                   abucpahs, and 7 parameters for dust sizes: exponent
c                   of size distribution qmrn; upper radii of grain
c                   components alec, alesi < 3.442e-5, arad_polmin_aC,
c                   arad_polmin_Si, arad_polmax, and aled (= largest
c                   radius of dark dust particles)
c                   w12_vv2_Ralf_283wave.dat: wavelength grid of dust
c                   model jsmTaufit.inp: tauV, Ebv_obs of star used in
c                   Eq.2 and 3 as in B)
c     
c     d.Q* files   : Grain efficiencies Q are stored for grain components in
c                   d.QSi_X922E028V005_idg60, d.QellipDark, d.QellipaC, d.Qmie_vGr,
c                   d.Qmie_vSi
c
c     Output: in directory ./Output/
c     Kappa.out    : Optical for absorption (sa_*) and scattering (ss_*) of
c                    the various grains for wavelength grid
c     Kappa4fit.out: Same as Kappa.out for wavelength grid adjusted for
c                    observed wavelength ranges when fitting reddening
c                    curves and as used by mp_absreDD.pro
c     PolKappa.out : Polarization cross sections sigp_* of large aC and
c                    Si grains; in this version, dark dust is spherical
c                    hence pol cross section sigp_dark=0, last column
c                    total dust extinction cross section (sigt=K [cm^2/g-dust]).
c     tau4fit.out  : Extinction, normalized, and absolute reddening curve
c
c     Notation:
c     Cross sections K (cm^2/g-dust) of the various components are
c     stored in variables sig* and later converted to optical depth by
c     multiplication of Nl, Nd (Your may set nl=nd=1 to keep K).
c
c     Version:
c     01.XX.XX cds release if accepted 
c     ------------------------------------------------------------------
c
      parameter(nfo=283)
      include "crossec.com"
...
c    Default parameter:
         ...
         arad_polmin_aC = 1.43e-05
         arad_polmin_Si = 0.76e-05 
         arad_polmax    = aled
         abuc           = 88. 
         abusi          = 15.
         abucvgr        = 6.
         ...
         fDark          = 0.55d0 ! fraction of dust mass in Dark dust
         write(6,*) ' ! fraction of dust mass in Dark dust:          fDark  = ', fDark 
         zcpahs         = 30
         zcpahb         = 0 
         hydpahs        = 0.4
         hydpahb        = 0.2
         tauV           = 1.72  ! depends on Star see ./Input/jsm12fit.inp
         ebv_obs        = 0.32
         rdust          = 1.5d12
         totlum         = 3.85d33
         tstar          = 6000.
...
c ------------------------------------------------------------------       
c   Using mpfit: reading in input parameters from ./Input/jsm12fit.inp and ./Input/jsmTaufit.inp
c
...
c Interpolate Qs of the spheroids to  wavelenght grid of the dark dust model 
       do  k = 1, mm
...
c
c     Interpolate to new wavelenght grid for Dark dust
c
...
c        b) Dark dust as separate component with optical depth: tau_d and cross sect sigt_d
         ...
         call sigtDark_ebv
         write(6,'(a60,2f7.2)') ' For fdark = Mdark/(Mmrn+Mdark) recompute tauV: ', fdark, tauv
