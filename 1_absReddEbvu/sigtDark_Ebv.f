      subroutine sigtDark_Ebv

      include "crossec.com"

      
c     Computes Av for given E(B-V) by assumnig that about half (=fdark
c     in %) of the mass is in submicron grains.  Rel. mass of MARNdust
c     in 1g of aC, Si, pah, vgr, vsi and Dark dust. Outp[ut is the total
c     extinction cross section of larg+nano+dark + Dark dust : sigt*
c     (cm^2/g-dust ISM)
c
...
c     xnl estimated from reddening E(B-V) converted to tbv
      
      xnl  = tbv / (sdb - sdv + slb - slv)
      xnl  = max(xnl,0.)
      xnd  = xnl * fdark/(1-fdark)

      tbv_mod = xnl*(slb-slv) + xnd*(sdb-sdv)
      
      
      if(ibug .ge. 2) then
       write(6,'(a40, 1p2e10.3)') ' slv, slv  *xnl = ', slv, slv*xnl
       write(6,'(a40, 1p2e10.3)') ' sdv, sdv  *xnd = ', sdv, sdv*xnd
       write(6,'(a40, 1p2e10.3)') ' slb, slb  *xnl = ', slb, slb*xnl
       write(6,'(a40, 1p2e10.3)') ' sdb, sdb  *xnd = ', sdb, sdb*xnd
       write(6,'(a40, 1p2e10.3)') ' (slb-slv) *xnl = ', xnl*(slb-slv)
       write(6,'(a40, 1p2e10.3)') ' (sdb-sdv) *xnd = ', xnd*(sdb-sdv)
       write(6,'(a40, 1p2e10.3)') ' obs Ebv/1.086 = tbv = ', tbv, tbv_mod
      endif
...
c      recomputed tauV assuminh 50% in dark dust
c     
        sigtV  = sigt(kvis)
        tauV   = sigtV
...
c     check precission in using nl, nd in renormalization of sig*
       if (abs(Ebv_obs/Ebv-1.).gt. 0.001)
     $     write(6,'(a35, 0p3f10.3)') ' Ebv_obs, Ebv_mod, err(obs/mod) = ',
