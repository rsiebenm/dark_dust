; this computes Av(Dgaia) and the fits to the reddening curve calling mp_absredd
; with: 
; INPUT: 
...
;  Dgaia   : trigonometric distance as such derived from Gaia parallax
...
;
; External:    ./a.crossec_dd (compile crossec_AvEbv.f)
;
; -------------------------------------------------
;
...
;  if star(i) eq 'HD129557' or star(i) eq   'HD146285' or star(i) eq 'HD315023' then begin
   Vmag    = vk(i)
   Ebv_ref = av(i)/Rv(i)
   Mv_mag  = Mv(i)
   Dgaia   = Dp(i)
